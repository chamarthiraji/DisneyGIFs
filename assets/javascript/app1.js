	var topics = ["Aladdin","Alice","Ariel"," Olaf","Goofy","Belle","pocahontas","Anna","Elsa","Tinkerbell"];
	...
	function displayMovieInfo(){
		...
		var queryURL = "http://api.giphy.com/v1/gifs/search?q="+images+"&api_key=dc6zaTOxFJmzC ";
		if ( imagesCreated ) {
			removeImages();
		}	
			...
			method: 'GET'
				...
				$('#gifimages_view').hide();
     			...
     			for (var i = 0; i < searchedImagesLength ; i++) {
                    ...
                    var gifDiv = $('<div  class="item col-xs-6 col-md-4 ">');
                    var rating = results[i].rating;
                    var p = $('<p>').text("Rating: " + rating);
                    console.log("img src : ", results[i].images.fixed_height_still.url);
                    var personImage = $('<img  id="imageid"+i class="image img-responsive">');
